...
- 네 운영 조회는 독립적으로 로드한다. 한 API가 실패해도 나머지 결과를 표시하고, 실패한 조회의
  이름과 오류 메시지를 안내한다.
- 응답에는 인증 비밀값, 비밀번호, refresh token을 포함하지 않는다.
- 자동 결석 처리로 생성된 출석 기록은 `checked_by`가 `NULL`일 수 있다. 관리자 조회 응답은 이를
  `checkedBy: null`로 보존하며 `0` 같은 대체 ID로 변환하지 않는다.
