...
| Part4 `attendance` | 일정 식별자·응답 정책·그룹 전체 출석률 권한 정책을 제공하고 출석 판정·집계는 Part4에 맡긴다. 그룹 전체 출석률 집계의 일정 범위·활성 그룹원 목록 조회는 1차 MVP에 한해 직접 Repository 조회를 허용한다. |
| `activity` | 일정 식별자를 제공하되 활동 기록 규칙은 담당 영역에 맡긴다. |
| `global` | 공통 응답·예외·보안 변경은 담당자 합의와 리뷰를 받는다. |

다른 영역의 Entity나 Repository에 직접 결합하지 않는다. 단, Part4 그룹 전체 출석률 집계는 일정 범위와 활성 그룹원 목록을 구성하기 위해 `StudyScheduleRepository`, `GroupMemberRepository`를 직접 조회할 수 있는 1차 MVP 예외다. 이 예외에서는 권한·상태 판단을 직접 구현하지 않고 `StudyGroupAttendanceRatePolicyReader`를 사용한다. 외부 변경이 필요하면 공개 경계로 해결 가능한지 먼저 확인하고, 담당자와 계약을 합의한 뒤 최소 범위와 테스트만 수정한다. 독립 검토가 가능하면 별도 브랜치·PR로 분리하고, 같은 PR이면 경로·이유·영향·검증 결과를 명시한다.
...
세부 데이터 계약은 `erd.md`, HTTP·내부 서비스·오류 계약은 `api.md`, 사용자 기능·권한 규칙은 `feature-spec.md`를 따른다.
