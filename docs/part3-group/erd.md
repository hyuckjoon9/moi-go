...
| 일정 → 참석 응답 | `CASCADE` | `CASCADE` | 미래 일정 삭제 시 아직 확정 이력이 아닌 참석 응답도 함께 삭제된다. |
| 일정 → 출석 기록 | `RESTRICT` | `CASCADE` | 출석 기록과 누적 출석률을 보존하기 위해 일정 삭제를 막는다. |
| 일정 → 활동 기록 | `RESTRICT` | `CASCADE` | 활동 이력을 보존하기 위해 일정 삭제를 막는다. |

일정 삭제는 미래 일정에만 허용한다. 참석 응답만 있으면 함께 삭제하고, 출석 기록이나 활동
기록이 하나라도 있으면 삭제를 거부한다. 세부 권한, 시간 경계와 파트 간 공개 계약은
